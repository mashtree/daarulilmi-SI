...
class Bootstrap {
    ...
    public function __construct($registry) {
        ...
        //$this->role = ($logged) ? (Session::get('role') == ADMIN ? 'admin' : (Session::get('role') == USTADZ ? 'ustadz' : (Session::get('role') == WALI ? 'wali' : 'guest'))) : 'guest';
    }
	
	private function getRole(){
		$logged = Session::get('loggedin');
		if($logged){
			foreach($this->registry->roleAuth as $key=>$value){
				if(Session::get('role')==$value) return $value;
			}
		}else{
			return 'guest';
		}
